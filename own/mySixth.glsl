void main() {
    ...
    for (int i = 1; i < 223; i++) {
        ...
        uv0.y += 0.1 / power * tan(power * uv0.x + iTime);
    }
    ...
    col.r = smoothstep(0.9, 0.0, sin(uv.x * 90.0 + iTime));
    col.r *= sin(uv.y * 80.0 + iTime);
    col.r *= sin(uv.x * 80.0 + iTime);
    col.r *= sin(uv.y * 90.0 + iTime);
    col.r *= sin(uv.x * 90.0 + iTime);
    col.r *= sin(uv.y * 80.0 + iTime);
